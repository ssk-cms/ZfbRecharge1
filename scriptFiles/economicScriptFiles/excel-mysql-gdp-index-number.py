...
def open_excel():
    try:
        book = xlrd.open_workbook("国内生产总值指数100.xlsx")  # 文件名，把文件与py文件放在同一目录下
    except:
...
def search_count(cursor):
    ...
    select = "select count(id) from economic_gdp_index_number"  # 获取表中xxxxx记录数
...
def insert_deta(cursor):
    ...
    for i in range(7, sheet.nrows):  # 第一行是标题名，对应表中的字段名所以应该从第二行开始，计算机以0开始计数，所以值是1
        ...
        print(value)
        sql = "INSERT INTO economic_gdp_index_number(year, gdp,gross,first_produce,second_produce,third_produce,nong_lin_yu_muye,industry,construstion,pifa_lingshouye,traffic,stay,banking,estate,other,ave_gdp)VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
